...
if [ "$FREE_MB" -lt "$MIN_FREE_MB" ]; then
    ...
    while [ "$FREE_MB" -lt "$MIN_FREE_MB" ]; do
        ...
        OLDEST_FOLDER=$(find "$RECORD_DIR" -mindepth 2 -maxdepth 2 -type d 2>/dev/null | sort | head -1)
        ...
        if mountpoint -q "$SAMBA_PATH"; then
            ...
            # Perform rsync with checksum verification
            rsync -a --checksum "$OLDEST_FOLDER/" "$DEST/" >> "$LOG_FILE" 2>&1
